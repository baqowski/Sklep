...
function eliteEncrypt($string) 
{
		...
		if (!isset($_SESSION['zalogowany']))		
	{
     ...
     $select_idSklep = mysqli_query($conn, 
     "SELECT id FROM sklep WHERE id = '$id_sklep' LIMIT 1");
     $row2 = mysqli_fetch_assoc($select_idSklep);
     $id_sklep =  $row2['id'];
    ...
    echo 'test';
   if (isset($_POST['usun_z_koszyka']) && isset ($_POST['id_koszyk'])) {
   
      $id_koszyk = $_POST['id_koszyk'];  
 
       
      $delete = "DELETE  FROM koszykklientasklepu WHERE  
        koszykklientasklepu.id_k = '$id_koszyk'";
...
echo "<div id='total'>Całkowity koszt:" .$wiersz['SUM(cena)']. " zl</div> ";  
        ...
        { 
            ...
            echo "<img id='zdj' src=../img/".$row['img'].".jpg height='150' width='150'>";
            echo '<input type="hidden" value="'.$row['id_k'].'" name="id_koszyk">';
            echo '<input id="del" type="submit" value="Usuń z koszyka" name="usun_z_koszyka">';
            echo '</form>';            
        }
